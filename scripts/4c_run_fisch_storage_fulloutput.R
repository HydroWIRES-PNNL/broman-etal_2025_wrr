# ----
# Name: run_fisch_storage_fulloutput.R
...
# source 'fisch' scripts

source('/Users/brom374/Library/CloudStorage/OneDrive-PNNL/Documents/Projects/git/broman-etal_2025_wrr/fisch_temp/DP.R')
source('/Users/brom374/Library/CloudStorage/OneDrive-PNNL/Documents/Projects/git/broman-etal_2025_wrr/fisch_temp/helpers.R')
source('/Users/brom374/Library/CloudStorage/OneDrive-PNNL/Documents/Projects/git/fisch/R/run_fisch.R')
...
bc = bcs_storage %>% dplyr::filter(EIA_ID == 6400) # REMOVE
...
# start of run loop ------------------------------------------------------------
if(paste0('dayahead-full_storage_', EIA_ID, '.csv') %in% list.files('data/output_data/fisch/')){
...
# for(wk in 1:30){
  ...
  week_commencing = bc[wk,] %>% .[['week_commencing']]
    ...
    map(function(x){
      ...
      if(x == 'perfect'){
        if(wk == 1){
          ...
          s1 = simulation_collect_perfect_dayahead %>% 
            dplyr::filter(day == 7, time == 169, type == 'actual',
                          week_commencing == max(week_commencing, na.rm = TRUE)) %>%
            pull(storage_sim)
        }
      ...
      if(x == 'persistence'){
        if(wk == 1){
          ...
          s1 = simulation_collect_persistence_dayahead %>% 
            dplyr::filter(day == 7, time == 169, type == 'actual',
                          week_commencing == max(week_commencing, na.rm = TRUE)) %>%
            pull(storage_sim)
        }
      ...
      if(x == 'synthetic'){
        if(wk == 1){
          ...
          s1 = simulation_collect_synthetic_dayahead %>% 
            dplyr::filter(day == 7, time == 169, type == 'actual',
                          week_commencing == max(week_commencing, na.rm = TRUE)) %>%
            pull(storage_sim)
        }
      ...
      schedule_release(mode = 'day_ahead_full',
                       inflow = wk_inflow,
      ...
      return(
        x_out %>%
          mutate(week_commencing = !!week_commencing,
...
bind_rows(
  ...
  write_csv(paste0('data/output_data/fisch/dayahead-full_storage_', EIA_ID, '.csv'))
