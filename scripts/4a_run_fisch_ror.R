# ----
# Name: 4a_run_fisch_ror.R
# Author: S. Turner, PNNL
# Modified: D. Broman, PNNL
# Last Modified: 2023-08-16
# Description: [D3] runs FIScH for run-of-river facilities
...
# start of run loop ------------------------------------------------------------
if(paste0('dayahead_ror_', EIA_ID, '.csv') %in% list.files('data/output_data/fisch/')){
...
message('done already')
  ...
  disc_grid_storage <- seq(0,
                           snap_to_disc(plant_specs[['MaxVol']], disc_step), disc_step)
  
  res_cap <- last(disc_grid_storage)
  
  # target_storage_constant <- disc_grid_storage[as.integer(length(disc_grid_storage) * 0.95)]
  initial_storage <- snap_to_disc(bc[['storage']][1], disc_step)
  
  # this is initial storage for RoR
  tibble(
    time = 0,
...
bind_rows(
  ...
  write_csv(paste0('data/output_data/fisch/dayahead_ror_', EIA_ID, '.csv'))
